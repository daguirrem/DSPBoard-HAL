...
//  if (err != W25Qxx_ERR_NONE) while(1);
  ...
  while (1)
  {
      ...
      **/
      ...
      W25Qxx_Read (buff, 0x007FFFFF-3, sizeof(buff), &err);                 /* Read  Block64 127 and 128  "     Block64 Erase" */
      if (err != W25Qxx_ERR_NONE) while(1);
      ...
      W25Qxx_Read (buff, 0x0100FFFF-3, sizeof(buff), &err);                 /* Read  Block64 255 and 256  "     Block64 Erase" */
      if (err != W25Qxx_ERR_NONE) while(1);
      ...
      **/
      ...
      W25Qxx_Read (buff, 0x00007FFF-3, sizeof(buff), &err);                 /* Read  Block32 0 and 1      "Test              " */
      if (err != W25Qxx_ERR_NONE) while(1);
      ...
      W25Qxx_Read (buff, 0x00057FFF-3, sizeof(buff), &err);                 /* Read  Block32 10 and 11    "     Block32 Erase" */
      if (err != W25Qxx_ERR_NONE) while(1);
      ...
      **/
      ...
      W25Qxx_Read (buff, 0x00000FFF-3, sizeof(buff), &err);                 /* Read  Sector 0 and 1         "     Sector Erase" */
      if (err != W25Qxx_ERR_NONE) while(1);
      ...
      W25Qxx_Read (buff, 0x01FFEFFF-3, sizeof(buff), &err);                 /* Read  Sector 8190 and 8191   "Test             " */
      if (err != W25Qxx_ERR_NONE) while(1);
      ...
      **/
      ...
      W25Qxx_Erase_Security(W25Qxx_SECTORADDR(0x00004034), &err);         
      if (err != W25Qxx_ERR_PAGEADDRBOUND) while(1);

      W25Qxx_Read_Security(buff,  0x00004000, 0x100,      &err);  
      if (err != W25Qxx_ERR_BYTEADDRBOUND) while(1);      
      W25Qxx_Program_Security(ee, 0x000040F0, sizeof(ee), &err);            /* Error address bound */
      if (err != W25Qxx_ERR_BYTEADDRBOUND) while(1);
      W25Qxx_Program_Security(ee, 0x00004012, sizeof(ee), &err);
      if (err != W25Qxx_ERR_BYTEADDRBOUND) while(1);
      W25Qxx_Read_Security(buff,  0x00004000, 0x100,      &err);
      if (err != W25Qxx_ERR_BYTEADDRBOUND) while(1);
      ...
      **/
      ...
      W25Qxx_WBit_WPS(W25Qxx_VOLATILE, 1);          
      W25Qxx_Individual_Locked(0x00000000, &err);
      if (err != W25Qxx_ERR_NONE) while(1);
      if (W25Qxx_ReadLock(0x00000000) == 0x00) while(1);
      W25Qxx_WBit_WPS(W25Qxx_VOLATILE, 0);    
      if (W25Qxx_ReadLock(0x00000000) == 0x00) while(1);
      
      W25Qxx_WBit_WPS(W25Qxx_VOLATILE, 1);          
      W25Qxx_Individual_UnLock(0x00000000, &err);
      if (err != W25Qxx_ERR_NONE) while(1);
      if (W25Qxx_ReadLock(0x00000000) == 0x01) while(1);
      W25Qxx_WBit_WPS(W25Qxx_VOLATILE, 0);    
      if (W25Qxx_ReadLock(0x00000000) == 0x01) while(1);
